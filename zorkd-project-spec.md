...
The rate limits of section 8.1 key on the address the connection came from, and by default do not read `X-Forwarded-For` — an unconfigured header any client can write cannot be believed. Behind a proxy that leaves every player sharing one bucket, so name the proxy: `-trusted-proxies` (`ZORK_TRUSTED_PROXIES`) takes a comma-separated list of CIDRs, and a request whose direct peer is in that set is attributed to the first non-trusted address from the right of `X-Forwarded-For`, reading past the trusted hops to the real client. Reading from the right, and stopping at the edge of the trust, is what keeps a client from forging a prefix: anything left of the trusted hops it could have written itself. The set must name only proxies under the operator's control; a host trusted here can claim to be any client. Have the proxy set `X-Real-IP` and `X-Forwarded-For` so the header is arriving.
