...
Because that cost is unavoidable by design, and because both routes are reachable by anyone who can reach the server, `POST /login` and `POST /register` are rate limited in `internal/httpserver` — a token bucket per source address and a second per submitted address, since limiting one end leaves the other open. A refused attempt is answered with `429` and `Retry-After` on the route's own form, in words that read the same whether or not the address has an account. The source address is the direct peer unless `-trusted-proxies` names the fronting proxy, in which case it is read from `X-Forwarded-For`; section 17.1 covers that, because it is a deployment fact rather than a property of the limiter.
...
The rate limits of section 8.1 key on the address the connection came from, and by default do not read `X-Forwarded-For` — an unconfigured header any client can write cannot be believed. Behind a proxy that leaves every player sharing one bucket, so name the proxy: `-trusted-proxies` (`ZORK_TRUSTED_PROXIES`) takes a comma-separated list of addresses, and a request whose direct peer is one of them is attributed to the first untrusted address from the right of `X-Forwarded-For`, reading past the trusted hops to the real client. Reading from the right, and stopping at the edge of the trust, is what keeps a client from forging a prefix: anything left of the trusted hops it could have written itself. A hop that does not parse as an address ends the walk, and the request is attributed to the direct peer — coarse, but never a guess taken from a chain that has just proved unreadable.

Only a loopback host is accepted, bare or as an explicit `/32` or `/128`. The proxy is colocated by section 17.1's own requirements, so a trusted peer is always local, and the narrowness is enforced rather than documented because the wide value fails silently: a set reaching far enough to contain a client lets that client choose its own bucket, since the walk steps past every trusted hop and returns whatever lies to the left of it. Ranges are refused even when loopback, and `cmd/zorkd` will not start on a rejected value.

Trust remains opt-in. A loopback peer is not assumed to be a proxy, because in development the browser is on loopback as well, and inferring trust from the peer address would let anything running locally forge the header — the same hole, reached through the default. Have the proxy set `X-Real-IP` and `X-Forwarded-For` so the header is arriving.
