...
| `-trusted-proxies` | `ZORK_TRUSTED_PROXIES` | _(none)_ | comma-separated loopback addresses | which colocated proxies' `X-Forwarded-For` the rate limiter may believe |
...
`-trusted-proxies` names the proxies whose `X-Forwarded-For` the login and registration rate limits may believe. It is empty by default, and while it is, the forwarded header is ignored and every request is attributed to the address it arrived from — which behind a proxy is the proxy, so all players share one rate-limit bucket. Set it to the proxy's own address, `127.0.0.1` for nginx on the same host, and the limiter reads past the proxy to the real client.

**Only a loopback address is accepted**, bare or as an explicit `/32` or `/128`. zorkd expects its proxy beside it on one host, so a trusted peer is always local, and a range is refused even when it is itself loopback. That is a rule rather than advice because the wide value fails quietly: a set reaching far enough to contain a client lets that client name itself, since the limiter walks past every trusted hop and takes whatever it finds to the left, so the source limit turns off rather than merely coarsening. A proxy on another host is not a supported deployment and the server will not start with one configured.

Naming loopback is still something you have to do. The server does not assume a loopback peer is a proxy, because in development the browser is on loopback too, and inferring it would let anything local forge the header. See [Deployment](#deployment).
...
One consequence is worth handling rather than discovering. The rate limits on logging in and registering key on the address the connection came from, and by default do not read `X-Forwarded-For`, so behind a proxy every player would share one bucket. Set `-trusted-proxies` to the proxy's own address — `127.0.0.1` for the nginx below — and the limiter reads the forwarded chain past that hop to the real client. Only a loopback address is accepted, which is the deployment this section already requires: the proxy terminates TLS beside the server and talks to it over loopback. A proxy on another host would have to reach a listener that is not loopback, and that is the arrangement the rest of this section rules out.

The header is believed only from a peer in that set, and a host trusted there can set `X-Forwarded-For` to anything — so a proxy that passes a client's header through unchanged turns the source limit off. `$proxy_add_x_forwarded_for` below appends rather than passing through, which is what makes the chain readable. The example also sets `X-Real-IP`, so the header is already arriving.
...
server {
    ...
    location / {
        ...
        # -trusted-proxies 127.0.0.1; ignored otherwise.
